...
public final class AudioSpectrumAnalyzer: @unchecked Sendable {
    ...
    /// Analyzes raw pointer samples with SIMD vectorization and preallocated scratch memory.
    public func analyze(samples: UnsafePointer<Float>, count: Int, sampleRate: Float) -> AudioSpectrum {
        ...
        guard rms > 0.00001 else { return .zero }
        let level = min(1.0, max(0.0, rms * 12.0))

        // 2. Prepare windowed input slice
        if count >= fftSize {
            let offset = count - fftSize
            vDSP_vmul(samples + offset, 1, window, 1, inputScratch, 1, vDSP_Length(fftSize))
        } else {
            vDSP_vclr(inputScratch, 1, vDSP_Length(fftSize))
            vDSP_vmul(samples, 1, window, 1, inputScratch, 1, vDSP_Length(count))
        }

        // 3. Convert to split complex and execute in-place real-to-complex FFT
        ...
        // 5. Frequency band extraction (bins 20-250 Hz, 250-2500 Hz, 2500-8000 Hz)
        let df = sampleRate / Float(fftSize)
        let kb1 = max(1, Int(round(20.0 / df)))
        let kb2 = max(kb1, Int(round(250.0 / df)) - 1)
        let km1 = kb2 + 1
        let km2 = max(km1, Int(round(2500.0 / df)) - 1)
        let kt1 = km2 + 1
        ...
        // 6. Normalization and perceptual compression
        // In vDSP_fft_zrip, full-scale sine magnitude is N/2 = 256.0.
        let normFactor: Float = Float(halfSize)
        let bassNorm = bassPeak / normFactor
        let midNorm = midPeak / normFactor
        let trebleNorm = treblePeak / normFactor

        let bass = min(1.0, max(0.0, sqrt(bassNorm * 1.5)))
        let mid = min(1.0, max(0.0, sqrt(midNorm * 1.5)))
        let treble = min(1.0, max(0.0, sqrt(trebleNorm * 1.5)))
